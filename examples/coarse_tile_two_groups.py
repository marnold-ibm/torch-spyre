...
def f(x, y):
    # Two independent ops with different slice counts -> two separate groups
...
result_x, result_y = torch.compile(f)(x_dev, y_dev)
result_x = result_x.cpu()
result_y = result_y.cpu()

print(torch.abs(rx - result_x).amax())
print(torch.abs(ry - result_y).amax())