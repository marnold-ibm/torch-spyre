...
def resolve_hints(operations: list[Operation]) -> None:
    ...
    op.nested_hints: one inner list per hint scope (outermost first), each
    containing a DimHint for every loop variable that was named in that scope.
    """
    for op in operations:
        ...
        # DimHint and file it in the correct level bucket.
        by_level: list[list[DimHint]] = [[] for _ in levels]
        for i, sym in enumerate(op.loop_var_dims):
            ...
            by_level[level_idx].append(
                DimHint(
                    dim_names=hinted_names,
                    range_size=all_ranges.get(sym, 0),
                    split_count=split_count,
                    dim_index=i,
                    is_reduction=any(name in reduction_dims for name in hinted_names),
                )
            )
        op.nested_hints = by_level

    if hints_logger.isEnabledFor(logging.INFO):
        ops = [
            op
            for op in operations
            if isinstance(op, ComputedBuffer) and getattr(op, "nested_hints", None)
        ]
        if ops:
            hints_logger.info("=== resolve_hints ===")
            for op in ops:
                hints_logger.info(f"{op.get_operation_name()}:")
                for level_idx, level_hints in enumerate(op.nested_hints):
                    for h in level_hints:
                        per_tile = (
                            h.range_size // h.split_count if h.range_size else "?"
                        )
                        reduction_tag = "  [reduction]" if h.is_reduction else ""
                        hints_logger.info(
                            f"  level={level_idx}  {h.dim_names}  range={h.range_size}"
                            f"  split_count={h.split_count}  -> {per_tile} per tile"
                            f"  dim_index={h.dim_index}{reduction_tag}"
                        )


def _group_spec(nested_hints: list[list[DimHint]]) -> list[tuple]:
    """Build the coarse_tile() spec from op.nested_hints.

    Returns a list of (K, tiled_dims) tuples, one per hint level, outermost
    first.  Reduction dims are excluded — coarse tiling only tiles compute dims.
    """
    result = []
    for level in nested_hints:
        tiling_dims = [h for h in level if not h.is_reduction]
        if not tiling_dims:
            continue
        result.append(
            (
                sympy.Integer(tiling_dims[0].split_count),
                [h.dim_index for h in tiling_dims],
            )
        )
    return result
...
def hints_to_coarse_tile_groups(operations: list[Operation]) -> list[tuple]:
    """Build coarse_tile() groups from op.nested_hints (set by resolve_hints).
    ...
    def _key(op):
        ...
        resolved = getattr(op, "nested_hints", [])
        if not resolved:
            return None
        return (
            tuple(
                (level_idx, h.split_count, h.dim_index)
                for level_idx, level in enumerate(resolved)
                for h in level
    ...
    for op in operations:
        ...
        if key is not None and key == current_key:
            ...
            if current_ops and current_key is not None:
                spec = _group_spec(current_ops[0].nested_hints)
