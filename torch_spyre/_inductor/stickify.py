...
def pointwise_layout(n: SchedulerNode, args: list[SchedNodeArg], permute_needed: dict) -> FixedTiledLayout:
    ...
    # Rewrite args with permutation needed 
    if n in permute_needed:
        for arg_info in permute_needed[n]["arg_infos"]:
            arg_index = arg_info["arg_num"]
            new_index = arg_info["new_index"]
            print (f"Rewriting arg {arg_index} with new index {new_index}")
            object.__setattr__(args[arg_index].dep, 'index', new_index)  
            permute_needed[n]["restickify_needed"] = False  # Clear the restickify needed flag since we have now rewritten the index

    if len(args) == 1:
        x = args[0]
        print ("MRA: Unary arg:", x)
        ...
        # index = args[1].dep.index
        # print ("MRA: ===========================================")
        # print ("MRA: index:", index, type(index))
        
        # syms = list(index.free_symbols)
        # c0 = next(s for s in syms if s.name == 'c0')
        # c1 = next(s for s in syms if s.name == 'c1')
        # c0_coeff = index.coeff(c0)
        # c1_coeff = index.coeff(c1)  
        # new_index = index - c0_coeff*c0 - c1_coeff*c1 + c0_coeff*c1 + c1_coeff*c0

        # print ("MRA: new_index:", new_index, type(new_index))
        # object.__setattr__(args[1].dep, 'index', new_index)  # TODO: Reconstruct
        # print ("ARGS:", args)
        ...
        if len(stick_exprs) > 1:
            ...
            # Make a plan to permute the inputs, record that plan, and return None as layout trigger a rerun of this stickify on this node
            index = args[1].dep.index
            print ("MRA: ===========================================")
            print ("MRA: index:", index, type(index))
            ...
            permute_needed[n] = {"restickify_needed": True,
                                 "arg_infos": [{"arg_num": 1, "new_index": new_index, "restickify_needed": True}]}


            print ("MRA: Stick exprs:", stick_exprs)

            # stl = device_layout_like(args[0].layout, output.dtype)
            # # layout = FixedTiledLayout(output.device, output.dtype, output.size, [64, 1], stl)
...
def propagate_spyre_tensor_layouts(
    ...
    # Use queue so we can reprocess a node when needed
    queue = deque(nodes)
    ...
    while queue:
        ...
        if isinstance(n, SchedulerNode) and isinstance(n.node, ComputedBuffer):
            ...
            if isinstance(n.node, FallbackKernel):
                n = queue.popleft() if queue else None
                if not (
        ...
        if n in permute_needed and permute_needed[n].get("restickify_needed"):
            # Reprocessing was triggered, requeue this node
            queue.appendleft(n)
